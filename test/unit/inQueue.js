...
module.exports = (function() {
    ...
    describe('InQueue', function() {
        it('should create an async queue', function(done) {
            ...
            global.asyncQueue = new InQueueManager(
                tracker,
                0,
                {},
                queue,
            ...
            assert.equal(
                typeof global.asyncQueue.push,
                'function',
                'async queue is created'
            )

            assert.equal(
                global.testValue,
                15,
                'function originally stored in provided queue is executed when async queue is created'
            )
            done()
        })
        ...
        it('should create a second tracker and add both trackers\' attributes to output', function(done) {
            ...
            assert.equal(
                global.testValue,
                24,
                'Backward compatibility: Create a tracker using the legacy setCollectorUrl method'
            )
            done()
